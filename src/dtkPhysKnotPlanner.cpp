	...
	void dtkPhysKnotPlanner::KnotRecognition(vector<dtkIntersectTest::IntersectResult::Ptr>& intersectResults)
	{
		if( mDoKnotPlanning )
		{
			...
			for( dtkID i = 0; i < sortedCollisionSegments.size(); i++ )
			{
				...
				{
					continuousSegmentStart 
						= continuousSegmentEnd 
						= sortedCollisionSegments[i];
				}
			...
			{
				if( continuousSegments[i][1] - continuousSegments[i][0] > 1 
					&& continuousSegments[i+1][1] - continuousSegments[i+1][0] > 1
					&& continuousSegments[i+1][0] - continuousSegments[i][1] < 7 )
				{
		...
		for( dtkID i = 0; i < mKnots.size(); i++ )
		{
			...
			for( dtkID j = 0;  j < mSutureThread->GetNumberOfSegments(); j++ )
			{
				...
				dtkDouble3 trappedSegmentCenter = ( mSutureThread->GetMassPoint(j * 2)->GetPosition() 
					+ mSutureThread->GetMassPoint(j * 2 + 2)->GetPosition() ) * 0.5;
