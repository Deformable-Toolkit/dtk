...
guideWireSimulation::guideWireSimulation(QWidget *parent)
	: QMainWindow(parent)
{
	// setup ui
...
void guideWireSimulation::open()
{
	...
	fd->resize(600,400);
	//fd->setFilter(QDir(QString("*.s3m *.S3M *.S3m *.s3M")));
...
void guideWireSimulation::idle()
{
	...
	*/
	//yzk add
	//(OpenGLWidget->core)->ApplyExternalForce(1, dtkT3<double>(0, 0.1, 0));
